...
# PHY
#------------------------------------------------------------------------
Phy/WirelessPhy set CPThresh_   10000.0         ;# used at MAC, this value basically disables capture 
Phy/WirelessPhy set CSThresh_   2.44547e-08     ;# 20% pkts sensed at 30m
Phy/WirelessPhy set RXThresh_   2.44547e-08     ;# 70% pkts received at 10m
Phy/WirelessPhy set Pt_         0.28183815      ;# watts

# MAC
#------------------------------------------------------------------------
Mac/ExOR set BER_               0.00001
...
add-packet-header TCP IP LL MacExOR MacExORACK
...
for {set i 0} {$i < $opt(fdfnum)} {incr i 1} {
    ...
    if { $i == 0 } {
        $fdtcp($fdflowid)  ForwardList 0 3 -1 -1
        $fdsink($fdflowid) ForwardList 3 0 -1 -1
    }
    $fdtcp($fdflowid)  MacType 4 
    $fdsink($fdflowid) MacType 4
...
proc stop {} {
    global ns_ tracefd namtrace node_ opt  fdtcp fdsink rdtcp rdsink
        ...
        set fdflowid 0
        set fdsum 0.0
        set f_throughput  [open "results.txt" "a"]
        puts $f_throughput "================================================="
