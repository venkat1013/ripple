...
struct mcexor_ack_frame {
        ...
        inline static mcexor_ack_frame* access(const Packet* p) {
                return (mcexor_ack_frame*) p->access(offset_);
        }
};

struct hdr_mac_mcexor {
        ...
        static int offset_;
        inline static int& offset() { return offset_; }
        inline static hdr_mac_mcexor* access(const Packet* p) {
                return (hdr_mac_mcexor*) p->access(offset_);
        }
};
...
public:
        ...
        inline u_int32_t getHdrLen11() {
                return(sizeof(struct hdr_mac_mcexor) + ETHER_FCS_LEN);
        }
       ...
       inline u_int32_t getACKlen() {
               return(sizeof(struct mcexor_ack_frame));
       }
