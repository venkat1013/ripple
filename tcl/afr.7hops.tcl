...
for {set i 0} {$i < $opt(fnum)} {incr i 1} {
    ...
    if { $opt(hops)==2 } {
      if { $i == 0 } {
          $tcp($flowid)  ForwardList 0 1 2
          $sink($flowid) ForwardList 2 1 0
      }
    }
    if { $opt(hops)==3 } {
      if { $i == 0 } {
    ...
    if { $opt(hops)==4 } {
      if { $i == 0 } {
          $tcp($flowid)  ForwardList 0 1 2 3 4
          $sink($flowid) ForwardList 4 3 2 1 0
      }
    }
    if { $opt(hops)==5 } {
      if { $i == 0 } {
          $tcp($flowid)  ForwardList 0 1 2 3 4 5
          $sink($flowid) ForwardList 5 4 3 2 1 0
      }
    }
    if { $opt(hops)==6 } {
      if { $i == 0 } {
          $tcp($flowid)  ForwardList 0 1 2 3 4 5 6
          $sink($flowid) ForwardList 6 5 4 3 2 1 0
      }
    }
    if { $opt(hops)==7 } {
      if { $i == 0 } {
          $tcp($flowid)  ForwardList 0 1 2 3 4 5 6 7
          $sink($flowid) ForwardList 7 6 5 4 3 2 1 0
      }
    }
...
#     set f_cwnd  [open "out.ripple.cwnd.flow.$flowid.txt" "w"]
#     set f_throughput  [open "out.ripple.thru.flow.$flowid.txt" "w"]
#     set f_rtt  [open "out.ripple.rtt.flow.$flowid.txt" "w"]
#     close $f_cwnd 
#     close $f_throughput 
#     close $f_rtt 
# puts $flowid
