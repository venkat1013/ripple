...
set opt(rate)		 216
set opt(dataRate)        216Mb
set opt(basicRate)       54Mb
set opt(PLCPhdrRate)     216Mb
...
# PHY
#------------------------------------------------------------------------
#for shadowing model: 802.11a in door, frequency 5Ghz, path loss exponent 5, shadowing deviation 8, reference
#distance 50. 
Phy/WirelessPhy set CPThresh_   10000.0         ;# used at MAC, this value basically disables capture effect
# Phy/WirelessPhy set CSThresh_   2.57006e-09   ;# 50% pkts sensed at 50m
# Phy/WirelessPhy set CSThresh_   6.75247e-09     ;# 30% pkts sensed at 50m
# Phy/WirelessPhy set CSThresh_   8.84717e-12   ;# 99% pkts sensed <= 100m
...
# MAC
#------------------------------------------------------------------------
Mac/ExOR set BER_               0.0000  ;# not useful, shadowing model is good enough
#Mac/ExOR set BER_               0.00001 ;#per=8% for TCP data, 1% for TCP ACKs
#Mac/ExOR set BER_               0.0001 ;#per=58% for TCP data, 7% for TCP ACKs
# Mac/ExOR set BER_               0.00005 ;#per=35% for TCP data, 4% for TCP ACKs
Mac/ExOR set basicRate_         $opt(basicRate)
Mac/ExOR set dataRate_          $opt(dataRate)
Mac/ExOR set CWMin_             $opt(CWmin)
Mac/ExOR set CWMax_             $opt(CWmax)
Mac/ExOR set SlotTime_          $opt(SlotTime)
Mac/ExOR set SIFS_              $opt(SIFS)
Mac/ExOR set ShortRetryLimit_   $opt(ShortRetryLimit)
Mac/ExOR set LongRetryLimit_    $opt(LongRetryLimit)
Mac/ExOR set RTSThreshold_      $opt(RTSThreshold)
Mac/ExOR set MAX_Sq_SIZE_       $opt(ifqlen)
Mac/ExOR set num_sta_           $opt(nn)
...
for {set i 0} {$i < $opt(fdfnum)} {incr i 1} {
    ...
    set f_cwnd  [open "out.fd.cwnd.flow.$fdflowid.txt" "w"]
    set f_throughput  [open "out.fd.thru.flow.$fdflowid.txt" "w"]
    set f_rtt  [open "out.fd.rtt.flow.$fdflowid.txt" "w"]
    close $f_cwnd 
    close $f_throughput 
    close $f_rtt 
...
proc stop {} {
    global ns_ tracefd namtrace node_ opt  fdtcp fdsink rdtcp rdsink
        set now [$ns_ now]

        set fdflowid 0
        set fdsum 0.0
        set f_throughput  [open "results.real.topo.txt" "a"]
        puts $f_throughput "================================================="
