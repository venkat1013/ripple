...
set opt(mac)            Mac/MCExOR                   ;# MAC type
...
# MAC
#------------------------------------------------------------------------
Mac/MCExOR set basicRate_         $opt(basicRate)
Mac/MCExOR set dataRate_          $opt(dataRate)
Mac/MCExOR set CWMin_             $opt(CWmin)
Mac/MCExOR set CWMax_             $opt(CWmax)
Mac/MCExOR set SlotTime_          $opt(SlotTime)
Mac/MCExOR set SIFS_              $opt(SIFS)
Mac/MCExOR set ShortRetryLimit_   $opt(ShortRetryLimit)
Mac/MCExOR set LongRetryLimit_    $opt(LongRetryLimit)
Mac/MCExOR set RTSThreshold_      $opt(RTSThreshold)
Mac/MCExOR set MAX_Sq_SIZE_       $opt(ifqlen) 
Mac/MCExOR set num_sta_           $opt(nn)
...
add-packet-header TCP IP LL MacMCExORData MacMCExORAck
...
#     }
    ...
    $tcp($flowid)  MacType 4 
    $sink($flowid) MacType 4


    set app($flowid) [new Application/FTP]
    $app($flowid) attach-agent $tcp($flowid)

    set f_cwnd  [open "out.mcexor.cwnd.flow.$flowid.txt" "w"]
    set f_throughput  [open "out.mcexor.thru.flow.$flowid.txt" "w"]
    set f_rtt  [open "out.mcexor.rtt.flow.$flowid.txt" "w"]
    close $f_cwnd 
    close $f_throughput 
    close $f_rtt 
# puts $flowid
...
proc stop {} {
    global ns_ tracefd namtrace node_ opt  tcp sink  
        ...
        set flowid 0
        set tempsum 0.0
        set f_throughput  [open "results.mcexor.multiflow.txt" "a"]
        puts $f_throughput "================================================="
        puts $f_throughput "MCExOR"
        puts $f_throughput "================================================="
...
proc print_stuff { } {
	...
	for {set i 0} {$i < $opt(fnum)} {incr i} {
		...
		set f_cwnd  [open "out.mcexor.cwnd.flow.$flowid.txt" "a"]
                set f_throughput  [open "out.mcexor.thru.flow.$flowid.txt" "a"]
                set f_rtt  [open "out.mcexor.rtt.flow.$flowid.txt" "a"]
