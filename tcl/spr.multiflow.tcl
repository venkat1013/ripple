...
for {set i 0} {$i < $opt(fnum)} {incr i 1} {
    ...
    if { $i == 1 } {
        $tcp($flowid)  ForwardList 0 4 -1 -1
        $sink($flowid) ForwardList 4 0 -1 -1
    }
    if { $i == 2 } {
        $tcp($flowid)  ForwardList 5 7 -1 -1
        $sink($flowid) ForwardList 7 5 -1 -1
    }


    $tcp($flowid)  MacType 1 ;# // RIPPLE=0, LPR=1, SPR=2, ETX=2
    ...
    set f_cwnd  [open "out.lpr.cwnd.flow.$flowid.txt" "w"]
    set f_throughput  [open "out.lpr.thru.flow.$flowid.txt" "w"]
    set f_rtt  [open "out.lpr.rtt.flow.$flowid.txt" "w"]
    close $f_cwnd 
    close $f_throughput 
    close $f_rtt 
...
proc stop {} {
    global ns_ tracefd namtrace node_ opt  tcp sink  
        ...
        puts $f_throughput "  sum: [string range $tempsum 0 3]"
        close $f_throughput 
...
proc print_stuff { } {
        ...
        for {set i 0} {$i < $opt(fnum)} {incr i} {
                ...
                set f_cwnd  [open "out.lpr.cwnd.flow.$flowid.txt" "a"]
                set f_throughput  [open "out.lpr.thru.flow.$flowid.txt" "a"]
                set f_rtt  [open "out.lpr.rtt.flow.$flowid.txt" "a"]
