...
set stop 10
        ...
        puts $f_throughput " "
        puts $f_throughput "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
        puts $f_throughput "SPR 7HOPS: $numflow"
        puts $f_throughput "SPR 7HOPS: $BER"
        puts $f_throughput "SPR 7HOPS: $hops"
 ...
 for {set i 0} {$i < [llength $BER] } {incr i} {
     for {set j 0} {$j < [llength $numflow] } {incr j} {
         for {set k 0} {$k < [llength $hops] } {incr k} {
            exec ns spr.7hops.tcl $stop $rate $basicrate  [lindex $numflow $j] [lindex $BER $i] [lindex $hops $k] >> "auto-result.txt" 2>> "auto-result.txt"
         }
    }
 }
