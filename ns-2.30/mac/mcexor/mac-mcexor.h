...
#ifndef ns_mac_mcexor_h
#define ns_mac_mcexor_h
...
struct mcexor_mac_data_hdr {
        ...
        struct mcexor_data_hdr  mcexor_data_hdr;
        u_char                  dh_body[1]; // size of 1 for ANSI compatibility

        // Header access methods
        static int offset_;
        inline static int& offset() { return offset_; }
        inline static mcexor_mac_data_hdr* access(const Packet* p) {
                return (mcexor_mac_data_hdr*) p->access(offset_);
        }
};


struct mcexor_mac_ack_hdr {
        struct frame_control    dh_fc;
        int                     dh_duration;
        u_char                  dh_ra[ETHER_ADDR_LEN];
        u_char                  dh_ta[ETHER_ADDR_LEN];
        u_char                  dh_3a[ETHER_ADDR_LEN];
        u_int16_t               dh_scontrol;
        struct mcexor_ack_hdr   mcexor_ack_hdr;
        u_char                  dh_body[1]; // size of 1 for ANSI compatibility

        // Header access methods
        static int offset_;
        inline static int& offset() { return offset_; }
        inline static mcexor_mac_ack_hdr* access(const Packet* p) {
                return (mcexor_mac_ack_hdr*) p->access(offset_);
        }
...
class PHY_MIB_MCEXOR {
public:
        PHY_MIB_MCEXOR(MacMCExOR *parent);
        ...
        inline u_int32_t getACKlen() {
                return(sizeof(struct mcexor_mac_ack_hdr));
        }
...
class MAC_MIB_MCEXOR {
public:

        MAC_MIB_MCEXOR(MacMCExOR *parent);
...
class MacMCExOR : public Mac {
        friend class    DeferTimerMCExOR;
        friend class    BackoffTimerMCExOR;
        friend class    IFTimerMCExOR;
        friend class    NavTimerMCExOR;
        friend class    RxTimerMCExOR;
        friend class    TxTimerMCExOR;
public:
        MacMCExOR();
...
protected:
        PHY_MIB_MCEXOR         phymib_;
        MAC_MIB_MCEXOR         macmib_;
       int     bss_id_;
...
private:
        ...
        IFTimerMCExOR           mhIF_;          // interface timer
        NavTimerMCExOR          mhNav_;         // NAV timer
        RxTimerMCExOR           mhRecv_;        // incoming packets
        TxTimerMCExOR           mhSend_;        // outgoing packets

        DeferTimerMCExOR        mhDefer_;       // defer timer
        BackoffTimerMCExOR      mhBackoff_;     // backoff timer
