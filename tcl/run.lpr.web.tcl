...
set stop 10
        ...
        puts $f_throughput " "
        puts $f_throughput "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
        puts $f_throughput "LPR WEB: $numflow"
        puts $f_throughput "LPR WEB: $BER"
...
puts [lindex $BER $i]
#                 exec ns lpr.multiflow.tcl 10 6 6 [lindex $numflow $j] [lindex $BER $i] >> "auto-result.txt" 2> /dev/null
                exec ns lpr.web.tcl $stop $rate $basicrate  [lindex $numflow $j] [lindex $BER $i] >> "auto-result.txt" 2> /dev/null
        }
        set f_throughput  [open "results.web.txt" "a"]
        puts $f_throughput ""
        puts $f_throughput ""
