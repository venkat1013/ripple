...
add-packet-header TCP IP LL MacExORData MacExORAck
...
#     }
    ...
    set f_cwnd  [open "out.exor.cwnd.flow.$flowid.txt" "w"]
    set f_throughput  [open "out.exor.thru.flow.$flowid.txt" "w"]
    set f_rtt  [open "out.exor.rtt.flow.$flowid.txt" "w"]
    close $f_cwnd 
    close $f_throughput 
    close $f_rtt 
# puts $flowid
...
proc print_stuff { } {
	...
	for {set i 0} {$i < $opt(fnum)} {incr i} {
		...
		set f_cwnd  [open "out.exor.cwnd.flow.$flowid.txt" "a"]
                set f_throughput  [open "out.exor.thru.flow.$flowid.txt" "a"]
                set f_rtt  [open "out.exor.rtt.flow.$flowid.txt" "a"]
