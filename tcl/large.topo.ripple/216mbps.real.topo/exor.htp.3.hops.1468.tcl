...
# MAC
#------------------------------------------------------------------------
Mac/Ripple set BER_               0.000001  ;# not useful, shadowing model is good enough
#Mac/Ripple set BER_               0.00001 ;#per=8% for TCP data, 1% for TCP ACKs
#Mac/Ripple set BER_               0.0001 ;#per=58% for TCP data, 7% for TCP ACKs
# Mac/Ripple set BER_               0.00005 ;#per=35% for TCP data, 4% for TCP ACKs
...
for {set i 0} {$i < $opt(fdfnum)} {incr i 1} {
    ...
    if { $i == 0 } {
        $fdtcp($fdflowid)  ForwardList 0 1 2 3
        $fdsink($fdflowid) ForwardList 3 2 1 0
    }

    $fdtcp($fdflowid)  MacType 0 ;#   AFR=0, LPR=1, SPR=2, ETX=2, Ripple=3
    $fdsink($fdflowid) MacType 0
...
# MAC
#------------------------------------------------------------------------
Mac/802_11 set BER_               0.000001  ;# not useful, shadowing model is good enough
...
for {set i 0} {$i < 1} {incr i 1} {
    ...
    if { $i == 0 } {
        $htptcp($htpflowid)  ForwardList 4 5 -1 -1
        $htpsink($htpflowid) ForwardList 5 4 -1 -1
    }

    $htptcp($htpflowid)  MacType 1
    $htpsink($htpflowid) MacType 1

    $ns_ at $opt(start) "$htpapp($htpflowid) send 1000000" 
