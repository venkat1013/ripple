...
#   Application 
#------------------------------------------------------------------------
Agent/TCP/Sack1              set packetSize_   $opt(PktSize)
...
for {set i 0} {$i < $opt(fnum)} {incr i 1} {
    ...
    set sink($flowid) [new Agent/TCPSink/Sack1]
    if { $i == 0 } {
...
#     }
    ...
    $tcp($flowid)  MacType 0 ;#// RIPPLE=0, LPR=1, SPR=2, ETX=2
...
proc print_stuff { } {
	...
	for {set i 0} {$i < $opt(fnum)} {incr i} {
		...
		set f_cwnd  [open "out.ripple.cwnd.flow.$flowid.txt" "a"]
                set f_throughput  [open "out.ripple.thru.flow.$flowid.txt" "a"]
                set f_rtt  [open "out.ripple.rtt.flow.$flowid.txt" "a"]
